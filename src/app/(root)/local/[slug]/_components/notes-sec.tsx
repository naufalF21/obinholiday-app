...
const NotesSec = ({ data }: { data: DestinationInterface }) => {
	const [showItinerary, setShowItinerary] = useState(false);

	return (
		<div className="flex flex-col border border-gray-300 p-8 gap-6">
			<div className="flex flex-col md:flex-row justify-between items-center gap-3 md:gap-0">
				<h3 className="md:text-xl">Catatan Perjalanan</h3>
				<Button
					variant="outline"
					size="sm"
				...
				<Accordion type="multiple">
					<AccordionItem value="include">
						<AccordionTrigger className="text-base">
							Include ({data.notes?.include.length})
						</AccordionTrigger>
						<AccordionContent>
							<ul>
								{data.notes?.include.map((item, index) => (
									<li
										key={index}
					...
					<AccordionItem value="exclude">
						<AccordionTrigger className="text-base">
							Exclude ({data.notes?.exclude.length})
						</AccordionTrigger>
						<AccordionContent>
							<ul>
								{data.notes?.exclude.map((item, index) => (
									<li
										key={index}
										className="text-sm/8 tracking-wider flex items-baseline gap-2"
									>
